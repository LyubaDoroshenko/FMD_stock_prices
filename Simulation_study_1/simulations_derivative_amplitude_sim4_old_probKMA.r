...
for(jj in 1:4){
  ...
  for(motif in 1:nmotifs){
    ...
    plot(1,type='n',xlab='',ylab='v(t)',xlim=c(0,(length(curves$coeff_motifs[[motif]])-norder+1)*dist_knots),ylim=c(-35,50),yaxt='n',
    ...
    axis(2,at=seq(-35,50,5),labels=c(NA,-30,NA,-20,NA,-10,NA,0,NA,10,NA,20,NA,30,NA,40,NA,50),cex.axis=1.5,las=2)
  ...
  plot(seq(0,curves$fd_curves[[i]]$basis$rangeval[2],by=0.5),eval.fd(seq(0,curves$fd_curves[[i]]$basis$rangeval[2],by=0.5),curves$fd_curves[[i]],Lfdobj=0),
       type='l',ylim=c(-35,50),yaxt='n',xlab='',ylab='x(t)',main=substitute(bold("Curves with motifs, "~sigma*"="*s),list(s=as.character(sd_noise))),lty=i,cex.main=2,cex.axis=1.5,cex.lab=1.7)
