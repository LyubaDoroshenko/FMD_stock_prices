...
probKMA <- function(Y0,Y1=NULL,standardize=FALSE,transformed=FALSE,K,c,c_max=Inf,P0=NULL,S0=NULL,
  ...
  #for parallelization (default number of CPU cores -1). If worker_number=NULL, the function is run sequentially.
...
find_candidate_motifs <- function(Y0,Y1=NULL,K,c,n_init=10,name='results',names_var='',V_init=NULL,
  ...
  # worker_number: number of CPU cores to be used for parallelization (default number of CPU cores -1). If worker_number=NULL, the function is run sequentially.
  ...
  if(((len_mean/c_mean>10)&(length(K)*length(c)*n_init<40))|(length(K)*length(c)*n_init==1)){
    ...
    probKMA_options$worker_number=NULL
    if(worker_number>1){
...
cluster_candidate_motifs <- function(filter_candidate_motifs_results,motif_overlap=0.6,
  ...
  # worker_number: number of CPU cores to be used for parallelization (default number of CPU cores). If worker_number=NULL, the function is run sequentially.
...
smooth_locpoly_single_boundary <- function(rate_filled,degree=2,bandwidth=NULL,v0=TRUE,v1=TRUE){
  # smooth a single rate curve using local polynomials
  # rate_filled: vector with rates in the windows of the considered region, after filling gaps
  # degree: degree of local polynomial used
  # bandwidth: bandwidth to be used in local polynomial fitting 
  ...
  # the bandwidth is set to 1 for the last point, so that the last point is interpolated
  bandwidth=c(rep(bandwidth,length(rate_filled)-1),1)
  if(v0){
    rate_smoothed$v0=rep(NA,length(rate_filled))
    rate_smoothed$v0[index]=locpoly(index,rate_filled[index],drv=0,kernel='normal',degree=degree,bandwidth=bandwidth,
                                    gridsize=length(rate_filled),range.x=c(1,length(rate_filled)))$y[index]
...
.diss_d0_d1_L2_last_point <- function(y,v,w,alpha,transform_y=FALSE,transform_v=FALSE,last_point=FALSE){
  ...
  # modification here
  if(last_point){
    index_last=tail(which(!is.na(y_norm[[1]])),1)
    v_norm[[1]]=t(t(v_norm[[1]])+as.vector(y_norm[[1]][index_last,]-v_norm[[1]][index_last,]))
    v_norm[[2]]=v_norm[[2]]
  }
  .diss_L2 <- function(y,v,w){
    # Dissimilarity index for multidimensional curves (dimension=d).
...
.find_min_diss_positive_last_point <- function(y,v,alpha,w,c_k,d,use0,use1,transform_y=FALSE,transform_v=FALSE,last_point=FALSE){
  ...
  #s_rep=(1-(v_len-c_k)):(y_len-v_len+1+(v_len-c_k))
  #I want to consider only the positive shifts
  s_rep=1:(y_len-v_len+1+(v_len-c_k))
  y_rep=lapply(s_rep,
               function(i){
                 index=i-1+seq_len(v_len)
                 y_rep_i=list(y0=NULL,y1=NULL)
...
motifs_forecast=function(motifs_search_results,list_test,list_training_ts,closest,overlap,threshold_q=1,bandwidth_smooth=10){
  ...
  for(j in 1:length(motifs_search_results$V0)){
    found_motifs[[j]]=list(v0=as.matrix(motifs_search_results$V0[[j]]),v1=as.matrix(motifs_search_results$V1[[j]]))
  }
  ...
  for(i in 1:length(list_test)){
    ...
    used_motifs[[i]]=array(NA, dim=c(length_test,length(motifs_search_results$V0),5), 
                           dimnames = list(t=1:length_test,closest=paste0("closest",1:length(motifs_search_results$V0)), dim3=c("motif","shift","overlap","distance","prediction")))
    ...
    list_test_detrend[[i]] = list_test[[i]]$Price-trend_calculate(list_training_ts[[i]], t=length(list_training_ts[[i]])+1:(length_test))
    ...
    for(t in 1:(length_test)){
      ...
      list_new_training_detrend[[i]][[t]]=c(stocks_of_interest_detrend[[i]],list_test_detrend[[i]][0:(t-1)])
      
      #I smooth the whole time series
      list_test_to_smooth[[i]][[t]]=list_new_training_detrend[[i]][[t]]
      
      list_test_detrend_smoothed[[i]][[t]]=smooth_locpoly_single_boundary(list_test_to_smooth[[i]][[t]],degree=3,bandwidth=bandwidth_smooth,v0=TRUE,v1=TRUE)
      ...
      for(j in 1:length(motifs_search_results$V0)){
        observed[[i]][[t]][[j]]=list(y0=as.matrix(c(data_train_test_v0[[i]][[t]][(length(motifs_search_results$Y0[[i]])-length(found_motifs[[j]]$v0)+t+1):(length(motifs_search_results$Y0[[i]])-1+t)],NA)),
                                     y1=as.matrix(c(data_train_test_v1[[i]][[t]][(length(motifs_search_results$Y1[[i]])-length(found_motifs[[j]]$v1)+t+1):(length(motifs_search_results$Y1[[i]])-1+t)],NA)))
        
        min_diss[[i]][[t]][[j]]=.find_min_diss_positive_last_point(y=observed[[i]][[t]][[j]],v=found_motifs[[j]],alpha=0.9,use0=TRUE,use1=TRUE,w=1,c_k=ceiling(overlap*length(found_motifs[[j]][[1]])),d=1,transform_y=TRUE,transform_v=FALSE,last_point=TRUE)
        
        min_rescaled[[i]][[t]][[j]]=min(observed[[i]][[t]][[j]]$y0[min_diss[[i]][[t]][[j]][1]:(nrow(observed[[i]][[t]][[j]]$y0)-1)])
        max_rescaled[[i]][[t]][[j]]=max(observed[[i]][[t]][[j]]$y0[min_diss[[i]][[t]][[j]][1]:(nrow(observed[[i]][[t]][[j]]$y0)-1)])
      }  
      ...
      #save motif, shift and distance for all motifs (in order from the closest one)
      order_index=order(diss_shift[[i]][[t]][,"distance"])
      used_motifs[[i]][t,,]=cbind(order_index,diss_shift[[i]][[t]][order_index,"shift"],NA,diss_shift[[i]][[t]][order_index,"distance"],NA)
      
      #compute the prediction given by each aligned motif
      for(j in 1:length(motifs_search_results$V0)){
        info_j = used_motifs[[i]][t,j,]
        
        motif_order_j = motifs_search_results$V0[[info_j["motif"]]]
        predictions_rescaled_min[[i]][[j]][[t]] = motif_order_j[length(motif_order_j)-info_j["shift"]+1]
        portion_of_motif_min[[i]][[j]][[t]] = motif_order_j[1:(length(motif_order_j)-info_j["shift"])]
        used_motifs[[i]][t,j,"overlap"] = length(motif_order_j)-info_j["shift"]
        ...
        min_y = min_unlist[[i]][[t]][info_j["motif"]]
        max_y = max_unlist[[i]][[t]][info_j["motif"]]
        predictions_fin_min[[i]][[j]][t]=(predictions_rescaled_min[[i]][[j]][[t]]-
                                            motif_order_j[length(motif_order_j)-info_j["shift"]])*(max_y-min_y)+
                                            tail(observed[[i]][[t]][[info_j["motif"]]]$y0,2)[1]
        used_motifs[[i]][t,j,"prediction"] = unlist(predictions_fin_min[[i]][[j]][t])
      }
    }
    
    #retain only the closest motifs which have distance in the lower threshold_q quantile of all distances
    threshold_dist = quantile(used_motifs[[i]][,,"distance"],probs = threshold_q)
    for(t in 1:length_test){
      closest_to_use = which(used_motifs[[i]][t,1:closest,"distance"]<threshold_dist)
      
      #use the closest motifs to compute prediction (weighted average based on 1/distance)
      predictions_average_final[[i]][[t]] = sum(used_motifs[[i]][t,closest_to_use,"prediction"]/used_motifs[[i]][t,closest_to_use,"distance"])/sum(1/used_motifs[[i]][t,closest_to_use,"distance"])
    }
    ...
    forecast[[i]] = as.matrix(predictions_fin_unlist[[i]]+trend_calculate(list_training_ts[[i]], t=length(list_training_ts[[i]])+1:(length_test)))
    
    #Using the naive forecast when no forecast was made using motifs
    t_naive[[i]]=which(is.nan(forecast[[i]]))
    forecast[[i]][t_naive[[i]]]=c(tail(list_training[[i]]$Price,1),list_test[[i]]$Price)[t_naive[[i]]]
  }
  return(list(forecast=forecast,t_naive=t_naive))
}
