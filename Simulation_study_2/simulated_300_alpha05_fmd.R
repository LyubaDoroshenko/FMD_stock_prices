...
dir.create("sim300_alpha05_fmd",showWarnings=FALSE)
...
#######################
set.seed(13333)
# use Sobolev-like distance d_0.1
diss = 'd0_d1_L2'
alpha = 0.5
...
files = list.files('./sim300_alpha05_fmd')#sim300_not_random is the name of the folder where
...
if('motifs_candidate.RData' %in% files){
  ...
  load('./sim300_alpha05_fmd/motifs_candidate.RData')
}else{
  ...
  find_candidate_motifs_results = find_candidate_motifs(Y0, Y1, K, c, n_init,V_init=V_init,
                                                        name = './sim300_alpha05_fmd/len300', names_var = 'x(t)',
  ...
  find_candidate_motifs_results = find_candidate_motifs(Y0, Y1, K, c, n_init,
                                                        name = './sim300_alpha05_fmd/len300', names_var = 'x(t)',V_init=NULL,
                                                        probKMA_options = list(c_max = c_max, standardize = FALSE, iter_max = 1000,
                                                                               iter4elong = iter4elong, trials_elong = trials_elong, max_gap = max_gap,
                                                                               return_options = TRUE, return_init = TRUE,
                                                                               diss = diss, alpha = alpha,transformed=TRUE),
                                                        plot = TRUE, worker_number = 1)
  save(find_candidate_motifs_results, file = './sim300_alpha05_fmd/motifs_candidate.RData')
}
...
pdf('./sim300_alpha05_fmd/clustering_candidate_motifs.pdf', height = 12, width = 9)
cluster_candidate_motifs_plot(cluster_candidate_motifs_results, ask = FALSE)# ,R_all=0.0002
dev.off()

### search selected motifs
motifs_search_results_05 = motifs_search(cluster_candidate_motifs_results, #R_all = 0.005, 
                                      use_real_occurrences = FALSE, length_diff = 0.3)#length_diff=+Inf


### plot FMD results (NB: no threshold of frequencies of motif found!)
pdf('./sim300_alpha05_fmd/motifs_search_results_05.pdf', height = 10, width = 20)
motifs_search_plot_norm(motifs_search_results_05, ylab = 'x(t)', freq_threshold = 5,
                        transformed=TRUE)
dev.off()

save(find_candidate_motifs_results, silhouette_average,
     filter_candidate_motifs_results,
     cluster_candidate_motifs_results, motifs_search_results_05,
     file='./sim300_alpha05_fmd/results_all.RData')
...
motifs_analysis=data.frame(c(1:length(motifs_search_results_05$V_length)),motifs_search_results_05$V_length,
                           motifs_search_results_05$V_frequencies,
                           motifs_search_results_05$R_motifs*1000,
                           c("user","random")[unlist(lapply(motifs_search_results_05$v_init,is.null))*1+1])
...
write_xlsx(motifs_analysis_ordered,"./sim300_alpha05_fmd/simulated_motifs_analysis.xlsx")
...
# plots for paper
pdf('./sim300_alpha05_fmd/motifs_search_results_05_for_paper_all_motifs.pdf', height = 4, width = 6)
motifs_search_plot_for_paper(motifs_search_results_05, freq_threshold = 5,
                             transformed=TRUE)
dev.off()
